...
use crate::{key_index, HttpsLayerSettings};
use antidote::Mutex;
...
where
    ...
    S::Error: Into<Box<dyn Error + Send + Sync>>,
    S::Future: Unpin + Send + 'static,
    S::Response: Read + Write + Connection + Unpin + fmt::Debug + Sync + Send + 'static,
{
    ...
    T: Read + Write + Unpin,
{
    fn poll_write(
        ...
        match &mut *self {
            ...
            MaybeHttpsStream::Https(s) => Pin::new(&mut TokioIo::new(s)).poll_write(ctx, buf),
        }
    }
    ...
    fn poll_flush(mut self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<io::Result<()>> {
        match &mut *self {
            ...
            MaybeHttpsStream::Https(s) => Pin::new(&mut TokioIo::new(s)).poll_flush(ctx),
        }
    }
    ...
    fn poll_shutdown(mut self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<io::Result<()>> {
        match &mut *self {
            ...
            MaybeHttpsStream::Https(s) => Pin::new(&mut TokioIo::new(s)).poll_shutdown(ctx),
        }
    }
}
